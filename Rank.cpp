...
void Rank_Initialize() {
	mImageHandle = LoadGraph("images/Rank.png");//画像のロード
	WaitTime = 0;
	Rank_Read();
}
//ランキング画面 終了処理
...
void Rank_Draw() {
	...
	SetFontSize(75);
	DrawString(250, 50, "RANKING", GetColor(255, 0, 0));
	// 文字の表示(点滅)
	if (++WaitTime < 50) {
		SetFontSize(16);
		DrawString(200, 460, "Escキーまたはxボタンを押すとメニュー画面に戻ります。",
...
void Rank_Input()
{
	...
	Ranking[(RANKCOUNT - 1)].score = Game_GetScore(); //ランキングデータの最後にスコアを登 録
